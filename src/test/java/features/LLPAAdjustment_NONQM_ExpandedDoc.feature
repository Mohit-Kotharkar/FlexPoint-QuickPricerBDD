Feature: FICO/LTV Loan Level Price Adjustments (LLPA) : CES NOO - NON-QM Expanded Doc
  As a user of the Quick Pricer
  I want to verify that the correct FICO/LTV LLPA adjustments are displayed

  Scenario Outline: Verify LLPA adjustment for FICO and LTV - NON-QM Expanded Doc
    Given I am on the Quick Pricer page
    Then I wait for 3 seconds
    When I select the program "NON-QM Expanded Doc"
    Then I wait for 5 seconds
    # Then I select the Income Doc Type "24 mo BS"
    # Then I select the Loan Term "30 Years"
    # Then I select the Purpose "Purchase"
    # Then I select the State "California (CA)"
    # Then I select the Citizenship "US Citizen"
    # Then I select the Property Type "Condo"
    # Then I select the # of Properties "1"
    # Then I select the # of Units "1"
    When I set FICO score to "<FICO>"
    And I set LTV to "<LTV>"
    Then I wait for 3 seconds
    Then the LLPA adjustment should have "<FICORange>" "<LTVRange>" "<Adjustment>"

    Examples:
      | FICORange | LTVRange | FICO | LTV | Adjustment |
    #   |      760+ |     0-50 |  770 |  50 |       +1.500 |
    #   |      760+ | 50.01-55 |  770 |  55 |       +1.500 |
    #   |      760+ | 55.01-60 |  770 |  60 |       +1.375 |
    #   |      760+ | 60.01-65 |  770 |  65 |       +1.250 |
    #   |      760+ | 65.01-70 |  770 |  70 |       +1.125 |
    #   |      760+ | 70.01-75 |  770 |  75 |       +0.750 |
    #   |      760+ | 75.01-80 |  770 |  80 |       +0.250 |
    #   |      760+ | 80.01-85 |  770 |  85 |       -1.375 |
    #   |      760+ | 85.01-90 |  770 |  90 |       -2.750 |
    #   | 740 - 759 |     0-50 |  750 |  50 |       +1.500 |
    #   | 740 - 759 | 50.01-55 |  750 |  55 |       +1.375 |
    #   | 740 - 759 | 55.01-60 |  750 |  60 |       +1.250 |
    #   | 740 - 759 | 60.01-65 |  750 |  65 |       +1.125 |
    #   | 740 - 759 | 65.01-70 |  750 |  70 |       +1.000 |
    #   | 740 - 759 | 70.01-75 |  750 |  75 |       +0.500 |
    #   | 740 - 759 | 75.01-80 |  750 |  80 |        0.000 |
    #   | 740 - 759 | 80.01-85 |  750 |  85 |       -1.875 |
    #   | 740 - 759 | 85.01-90 |  750 |  90 |       -3.000 |
    #   | 720 - 739 |     0-50 |  730 |  50 |       +1.375 |
    #   | 720 - 739 | 50.01-55 |  730 |  55 |       +1.250 |
    #   | 720 - 739 | 55.01-60 |  730 |  60 |       +1.125 |
    #   | 720 - 739 | 60.01-65 |  730 |  65 |       +1.000 |
    #   | 720 - 739 | 65.01-70 |  730 |  70 |       +0.625 |
    #   | 720 - 739 | 70.01-75 |  730 |  75 |       +0.250 |
    #   | 720 - 739 | 75.01-80 |  730 |  80 |       -0.500 |
    #   | 720 - 739 | 80.01-85 |  730 |  85 |       -2.125 |
    #   | 720 - 739 | 85.01-90 |  730 |  90 |       -4.000 |
    #   | 700 - 719 |     0-50 |  710 |  50 |       +1.250 |
    #   | 700 - 719 | 50.01-55 |  710 |  55 |       +1.125 |
    #   | 700 - 719 | 55.01-60 |  710 |  60 |       +0.875 |
    #   | 700 - 719 | 60.01-65 |  710 |  65 |       +0.625 |
    #   | 700 - 719 | 65.01-70 |  710 |  70 |       +0.375 |
    #   | 700 - 719 | 70.01-75 |  710 |  75 |       -0.125 |
    #   | 700 - 719 | 75.01-80 |  710 |  80 |       -1.125 |
    #   | 700 - 719 | 80.01-85 |  710 |  85 |       -2.500 |
    #   | 700 - 719 | 85.01-90 |  710 |  90 |       -4.500 |
    #   | 680 - 699 |     0-50 |  690 |  50 |       +0.750 |
    #   | 680 - 699 | 50.01-55 |  690 |  55 |       +0.625 |
    #   | 680 - 699 | 55.01-60 |  690 |  60 |       +0.375 |
    #   | 680 - 699 | 60.01-65 |  690 |  65 |        0.000 |
    #   | 680 - 699 | 65.01-70 |  690 |  70 |       -0.375 |
    #   | 680 - 699 | 70.01-75 |  690 |  75 |       -1.250 |
    #   | 680 - 699 | 75.01-80 |  690 |  80 |       -1.750 |
    #   | 680 - 699 | 80.01-85 |  690 |  85 |       -3.375 |
    #   | 680 - 699 | 85.01-90 |  690 |  90 |       -5.000 |
    #   | 660 - 679 |     0-50 |  670 |  50 |        0.000 |
    #   | 660 - 679 | 50.01-55 |  670 |  55 |        0.000 |
    #   | 660 - 679 | 55.01-60 |  670 |  60 |       -0.250 |
    #   | 660 - 679 | 60.01-65 |  670 |  65 |       -0.500 |
    #   | 660 - 679 | 65.01-70 |  670 |  70 |       -1.125 |
    #   | 660 - 679 | 70.01-75 |  670 |  75 |       -1.875 |
    #   | 660 - 679 | 75.01-80 |  670 |  80 |       -2.750 |
    #   | 660 - 679 | 80.01-85 |  670 |  85 | Not Eligible |
    #   | 660 - 679 | 85.01-90 |  670 |  90 | Not Eligible |
    #   | 640 - 659 |     0-50 |  650 |  50 |       -3.000 |
    #   | 640 - 659 | 50.01-55 |  650 |  55 |       -3.125 |
    #   | 640 - 659 | 55.01-60 |  650 |  60 |       -3.250 |
    #   | 640 - 659 | 60.01-65 |  650 |  65 |       -3.500 |
    #   | 640 - 659 | 65.01-70 |  650 |  70 |       -3.750 |
    #   | 640 - 659 | 70.01-75 |  650 |  75 |       -4.125 |
    #   | 640 - 659 | 75.01-80 |  650 |  80 |       -5.250 |
    #   | 640 - 659 | 80.01-85 |  650 |  85 | Not Eligible |
    #   | 640 - 659 | 85.01-90 |  650 |  90 | Not Eligible |

  Scenario Outline: Verify LLPA adjustment for DocType and LTV - NON-QM Expanded Doc
    Given I am on the Quick Pricer page
    Then I wait for 3 seconds
    When I select the program "NON-QM Expanded Doc"
    Then I wait for 5 seconds
    Then I select the Income Doc Type "<DocType>"
    And I set LTV to "<LTV>"
    Then I wait for 3 seconds
    Then the LLPA adjustment should have "<DocType>" "" "<Adjustment>"

    Examples:
      | DocType                      | LTVRange    | LTV | Adjustment   |
      |                     24 mo BS | <= 50%      |  50 |        0.250 |
      |                     24 mo BS | 50.01 - 55% |  55 |        0.250 |
      |                     24 mo BS | 55.01 - 60% |  60 |        0.250 |
      |                     24 mo BS | 60.01 - 65% |  65 |        0.250 |
      |                     24 mo BS | 65.01 - 70% |  70 |        0.250 |
      |                     24 mo BS | 70.01 - 75% |  75 |        0.250 |
      |                     24 mo BS | 75.01 - 80% |  80 |        0.125 |
      |                     24 mo BS | 80.01 - 85% |  85 |        0.000 |
      |                     24 mo BS | 85.01 - 90% |  90 |        0.000 |
      | CPA P&L w/ BS                | <= 50%      |  50 |       -0.375 |
      | CPA P&L w/ BS                | 50.01 - 55% |  55 |       -0.375 |
      | CPA P&L w/ BS                | 55.01 - 60% |  60 |       -0.500 |
      | CPA P&L w/ BS                | 60.01 - 65% |  65 |       -0.500 |
      | CPA P&L w/ BS                | 65.01 - 70% |  70 |       -0.500 |
      | CPA P&L w/ BS                | 70.01 - 75% |  75 |       -0.500 |
      | CPA P&L w/ BS                | 75.01 - 80% |  80 |       -0.625 |
      | CPA P&L w/ BS                | 80.01 - 85% |  85 |       -1.000 |
      | CPA P&L w/ BS                | 85.01 - 90% |  90 | Not Eligible |
      | CPA P&L w/o BS               | <= 50%      |  50 |       -0.750 |
      | CPA P&L w/o BS               | 50.01 - 55% |  55 |       -0.750 |
      | CPA P&L w/o BS               | 55.01 - 60% |  60 |       -1.000 |
      | CPA P&L w/o BS               | 60.01 - 65% |  65 |       -1.000 |
      | CPA P&L w/o BS               | 65.01 - 70% |  70 |       -1.000 |
      | CPA P&L w/o BS               | 70.01 - 75% |  75 |       -1.000 |
      | CPA P&L w/o BS               | 75.01 - 80% |  80 |       -1.250 |
      | CPA P&L w/o BS               | 80.01 - 85% |  85 | Not Eligible |
      | CPA P&L w/o BS               | 85.01 - 90% |  90 | Not Eligible |
      | Asset Util / WVOE / 12 mo BS | <= 50%      |  50 |        0.000 |
      | Asset Util / WVOE / 12 mo BS | 50.01 - 55% |  55 |        0.000 |
      | Asset Util / WVOE / 12 mo BS | 55.01 - 60% |  60 |        0.000 |
      | Asset Util / WVOE / 12 mo BS | 60.01 - 65% |  65 |        0.000 |
      | Asset Util / WVOE / 12 mo BS | 65.01 - 70% |  70 |        0.000 |
      | Asset Util / WVOE / 12 mo BS | 70.01 - 75% |  75 |        0.000 |
      | Asset Util / WVOE / 12 mo BS | 75.01 - 80% |  80 |        0.000 |
      | Asset Util / WVOE / 12 mo BS | 80.01 - 85% |  85 |        0.000 |
      | Asset Util / WVOE / 12 mo BS | 85.01 - 90% |  90 |        0.000 |

  Scenario Outline: Verify LLPA adjustment for Loan Purpose and LTV - NON-QM Expanded
    Given I am on the Quick Pricer page
    Then I wait for 3 seconds
    When I select the program "NON-QM Expanded Doc"
    Then I wait for 5 seconds
    Then I select the Purpose "<LoanPurpose>"
    And I set LTV to "<LTV>"
    Then I wait for 3 seconds
    Then the LLPA adjustment should have "<LoanPurpose>" "" "<Adjustment>"

    Examples:
      | LoanPurpose         | LTVRange    | LTV | Adjustment   |
      | Purchase            | <= 50%      |  50 |        0.000 |
      | Purchase            | 50.01 - 55% |  55 |        0.000 |
      | Purchase            | 55.01 - 60% |  60 |        0.000 |
      | Purchase            | 60.01 - 65% |  65 |        0.000 |
      | Purchase            | 65.01 - 70% |  70 |        0.000 |
      | Purchase            | 70.01 - 75% |  75 |        0.000 |
      | Purchase            | 75.01 - 80% |  80 |        0.000 |
      | Purchase            | 80.01 - 85% |  85 |        0.000 |
      | Purchase            | 85.01 - 90% |  90 |        0.000 |
      | Refinance           | <= 50%      |  50 |        0.000 |
      | Refinance           | 50.01 - 55% |  55 |        0.000 |
      | Refinance           | 55.01 - 60% |  60 |        0.000 |
      | Refinance           | 60.01 - 65% |  65 |        0.000 |
      | Refinance           | 65.01 - 70% |  70 |        0.000 |
      | Refinance           | 70.01 - 75% |  75 |        0.000 |
      | Refinance           | 75.01 - 80% |  80 |        0.000 |
      | Refinance           | 80.01 - 85% |  85 | Not Eligible |
      | Refinance           | 85.01 - 90% |  90 | Not Eligible |
      | Refinance C/O >=700 | <= 50%      |  50 |       -0.500 |
      | Refinance C/O >=700 | 50.01 - 55% |  55 |       -0.500 |
      | Refinance C/O >=700 | 55.01 - 60% |  60 |       -0.500 |
      | Refinance C/O >=700 | 60.01 - 65% |  65 |       -0.500 |
      | Refinance C/O >=700 | 65.01 - 70% |  70 |       -0.625 |
      | Refinance C/O >=700 | 70.01 - 75% |  75 |       -0.875 |
      | Refinance C/O >=700 | 75.01 - 80% |  80 |       -1.000 |
      | Refinance C/O >=700 | 80.01 - 85% |  85 | Not Eligible |
      | Refinance C/O >=700 | 85.01 - 90% |  90 | Not Eligible |
      | Refinance C/O <700  | <= 50%      |  50 |       -0.750 |
      | Refinance C/O <700  | 50.01 - 55% |  55 |       -0.875 |
      | Refinance C/O <700  | 55.01 - 60% |  60 |       -0.875 |
      | Refinance C/O <700  | 60.01 - 65% |  65 |       -0.875 |
      | Refinance C/O <700  | 65.01 - 70% |  70 |       -0.875 |
      | Refinance C/O <700  | 70.01 - 75% |  75 |       -0.875 |
      | Refinance C/O <700  | 75.01 - 80% |  80 | Not Eligible |
      | Refinance C/O <700  | 80.01 - 85% |  85 | Not Eligible |
      | Refinance C/O <700  | 85.01 - 90% |  90 | Not Eligible |

  Scenario Outline: Verify LLPA adjustment for Property Type and LTV - NON-QM Expanded Doc
    Given I am on the Quick Pricer page
    Then I wait for 3 seconds
    When I select the program "NON-QM Expanded Doc"
    Then I wait for 5 seconds
    Then I select the Property Type "<PropertyType>"
    And I set LTV to "<LTV>"
    Then I wait for 3 seconds
    Then the LLPA adjustment should have "<PropertyType>" "" "<Adjustment>"

    Examples:
      | PropertyType           | LTVRange      | LTV | Adjustment   |
      | Single Family Residence| <= 50%        |  50 |        0.000 |
      | Single Family Residence| 50.01 - 55%   |  55 |        0.000 |
      | Single Family Residence| 55.01 - 60%   |  60 |        0.000 |
      | Single Family Residence| 60.01 - 65%   |  65 |        0.000 |
      | Single Family Residence| 65.01 - 70%   |  70 |        0.000 |
      | Single Family Residence| 70.01 - 75%   |  75 |        0.000 |
      | Single Family Residence| 75.01 - 80%   |  80 |        0.000 |
      | Single Family Residence| 80.01 - 85%   |  85 |        0.000 |
      | Single Family Residence| 85.01 - 90%   |  90 |        0.000 |
      | Condo                  | <= 50%        |  50 |      -0.250  |
      | Condo                  | 50.01 - 55%   |  55 |      -0.250  |
      | Condo                  | 55.01 - 60%   |  60 |      -0.250  |
      | Condo                  | 60.01 - 65%   |  65 |      -0.250  |
      | Condo                  | 65.01 - 70%   |  70 |      -0.250  |
      | Condo                  | 70.01 - 75%   |  75 |      -0.250  |
      | Condo                  | 75.01 - 80%   |  80 |      -0.500  |
      | Condo                  | 80.01 - 85%   |  85 |      -0.500  |
      | Condo                  | 85.01 - 90%   |  90 | Not Eligible |
      | Non-Warrantable Condo  | <= 50%        |  50 |      -0.500  |
      | Non-Warrantable Condo  | 50.01 - 55%   |  55 |      -0.500  |
      | Non-Warrantable Condo  | 55.01 - 60%   |  60 |      -0.500  |
      | Non-Warrantable Condo  | 60.01 - 65%   |  65 |      -0.500  |
      | Non-Warrantable Condo  | 65.01 - 70%   |  70 |      -0.500  |
      | Non-Warrantable Condo  | 70.01 - 75%   |  75 |      -0.500  |
      | Non-Warrantable Condo  | 75.01 - 80%   |  80 |      -0.500  |
      | Non-Warrantable Condo  | 80.01 - 85%   |  85 | Not Eligible |
      | Non-Warrantable Condo  | 85.01 - 90%   |  90 | Not Eligible |
      | Condotel               | <= 50%        |  50 |      -1.000  |
      | Condotel               | 50.01 - 55%   |  55 |      -1.000  |
      | Condotel               | 55.01 - 60%   |  60 |      -1.000  |
      | Condotel               | 60.01 - 65%   |  65 |      -1.000  |
      | Condotel               | 65.01 - 70%   |  70 |      -1.000  |
      | Condotel               | 70.01 - 75%   |  75 |      -1.000  |
      | Condotel               | 75.01 - 80%   |  80 | Not Eligible |
      | Condotel               | 80.01 - 85%   |  85 | Not Eligible |
      | Condotel               | 85.01 - 90%   |  90 | Not Eligible |
      | 2 - 4 units            | <= 50%        |  50 |      -0.250  |
      | 2 - 4 units            | 50.01 - 55%   |  55 |      -0.250  |
      | 2 - 4 units            | 55.01 - 60%   |  60 |      -0.250  |
      | 2 - 4 units            | 60.01 - 65%   |  65 |      -0.250  |
      | 2 - 4 units            | 65.01 - 70%   |  70 |      -0.250  |
      | 2 - 4 units            | 70.01 - 75%   |  75 |      -0.250  |
      | 2 - 4 units            | 75.01 - 80%   |  80 |      -0.500  |
      | 2 - 4 units            | 80.01 - 85%   |  85 |      -0.500  |
      | 2 - 4 units            | 85.01 - 90%   |  90 | Not Eligible |
